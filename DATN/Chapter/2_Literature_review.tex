...
\begin{quote}
\small\ttfamily
[Loại vùng nhớ] [Kích thước] [Địa chỉ byte].[Địa chỉ bit]
\end{quote}

Trong đó: 
\begin{itemize}
    \item Loại vùng nhớ là I, Q, M, DB, T, C;
    ...
    \item Địa chỉ Byte để chỉ định Byte cụ thể trong phạm vi kích thước được chỉ định phía trên.
    \item Địa chỉ bit để chỉ định bit cụ thể trong Byte được chỉ định phía trên.
\end{itemize}


Vùng DB có cú pháp riêng:
\begin{quote}
\small\ttfamily
...
DWord: DB[n].DBD [byte]
\end{quote}
Ví dụ: \texttt{DB1.DBX0.0} là bit 0 của byte 0 trong DB1; \texttt{DB1.DBX0.1} là bit 1 của byte 0 trong DB1.

Siemens S7 dùng \textbf{big-endian} (byte có trọng số cao ở địa chỉ thấp).
...
\begin{figure}[H]
    ...
    \caption{Ngăn xếp giao thức S7comm (TPKT, COTP, S7 PDU)}
    \label{fig:s7-protocol-stack}
\end{figure}

\subsubsection{Phần COTP và TPKT}

Trong ISO TCP, để tương thích với TCP:

\begin{enumerate}
    \item TCP là giao thức hướng kết nối, ISO gốc không hướng kết nối $\rightarrow$ dùng \textbf{COTP} (\textit{Connection-Oriented Transport Protocol}) thiết lập kết nối logic trước khi trao đổi S7. Tham số \textbf{TSAP} (\textit{Transport Service Access Point}) định tuyến thông điệp tới đúng tiến trình trong CPU PLC.
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{|l|c|p{7cm}|}
            ...
            \hline
            Byte 1 (loại thiết bị) & \texttt{0x01}, \texttt{0x02}, \texttt{0x03} & PG (máy lập trình), OP (bảng vận hành), kết nối khác \\
            \hline
            Byte 2 (vị trí) & Rack/Slot & Vị trí CPU trong tủ điện (ví dụ Rack 0, Slot 2) \\
            \hline
        \end{tabular}
    \end{table}

    \item TCP là dạng luồng $\rightarrow$ dùng \textbf{TPKT} (\textit{ISO Transport Service on top of TCP}) đóng khung (\textit{framing}) để tách luồng byte thành gói có độ dài cố định. Ngăn xếp giao thức Siemens tự chèn header TPKT trước dữ liệu COTP và S7. Header TPKT dài 4 byte: byte 1 = \texttt{0x03} (phiên bản), byte 2 = \texttt{0x00} (reserved), 2 byte cuối = tổng độ dài gói (header + payload). Khi PLC nhận dòng byte từ TCP stream, nó đọc 4 byte đầu để biết cần nhận đủ bao nhiêu byte, rồi mới đẩy khối dữ liệu lên S7comm.
\end{enumerate}

Kết quả: giao thức vừa có độ dài thông điệp rõ ràng (ưu điểm ISO), vừa đi qua router IP (ưu điểm TCP). S7comm thường dùng cổng TCP \textbf{102}. COTP theo~\cite{rfc905}, TPKT theo~\cite{rfc1006,rfc2126}.

\subsubsection{Phần S7 PDU}

S7 thiết kế theo hướng \textit{function/command oriented}: mỗi lần truyền gồm một lệnh điều khiển hoặc phản hồi. Nếu lệnh không vừa một PDU, nó được chia thành nhiều PDU tiếp theo.
...
Một lệnh gồm \textbf{Header} và \textbf{Parameter/Data}:
...
\begin{itemize}
    \item \textbf{Protocol ID} (1 byte): luôn \texttt{0x32} để nhận dạng S7comm.
    \item \textbf{Message Type / ROSCTR} (\textit{Remote Operating Service Control}): cho biết loại gói tin. Bảng~\ref{tab:rosctr} liệt kê các giá trị phổ biến.
    \item \textbf{Reserved}: luôn \texttt{0x0000}, không mang ý nghĩa nghiệp vụ.
    \item \textbf{PDU Reference}: số tham chiếu do client tạo để định danh yêu cầu; khi PLC phản hồi sẽ trả lại số này để client ghép đúng cặp request--response. Số reference thường tăng dần theo từng yêu cầu mới.
    \item \textbf{Parameter length}, \textbf{Data length}: độ dài phần tham số và dữ liệu (big-endian).
    \item \textbf{Error class}, \textbf{Error code}: chỉ xuất hiện trong gói phản hồi \texttt{Ack\_Data} khi có lỗi.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|l|p{8.5cm}|}
        ...
        \hline
        1 & Job & Yêu cầu từ client tới server: read/write memory, read/write blocks, start/stop device, setup communication\ldots \\
        \hline
        2 & Ack & Xác nhận đã nhận yêu cầu, không kèm dữ liệu. \\
        \hline
        3 & Ack\_Data & Xác nhận đã nhận yêu cầu và kèm dữ liệu trả về cho job trước đó. \\
        \hline
        7 & User\_Data & Phần mở rộng so với giao thức gốc: \textit{Function Group} gồm programmer commands, cyclic data, block functions, CPU functions, security, time functions; mỗi nhóm lại có nhiều \textit{subfunction} (ví dụ trong CPU Functions: Read SZL, message service\ldots). \\
        \hline
    \end{tabular}
\end{table}

Phần \textbf{tham số} (tên tham số và giá trị nếu có) phụ thuộc \textit{Function Code} (\textit{S7 Header Function Code}) trong header. Các nhóm lệnh gồm Data Read/Write, Cyclic Data Read/Write, Directory info, System Info, Blocks move, PLC Control, Date and Time, Security, Programming\ldots

\subsection{Thiết lập kết nối S7comm}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Figure/image-1.png}
    \caption{Luồng thiết lập kết nối S7comm (minh họa Wireshark~\cite{wireshark_s7comm}, cổng TCP 102; nguồn mẫu: \textit{SampleCaptures/s7comm\_reading\_plc\_status.pcap})}
    \label{fig:s7-connect-flow}
\end{figure}

\subsubsection{Bắt tay TCP (three-way handshake)}

Client và PLC (ví dụ \texttt{192.168.1.10} và \texttt{192.168.1.40}) thiết lập kết nối TCP trên cổng 102 bằng ba bước SYN, SYN-ACK, ACK (\textit{three-way handshake}) --- trong bắt gói mẫu \textit{SampleCaptures/s7comm\_reading\_plc\_status.pcap} tương ứng các gói 5, 6, 7.

\subsubsection{Thiết lập kết nối COTP}

Sau khi kết nối TCP được thiết lập, client gửi \textbf{COTP CR} (\textit{COTP Connect Request}) tới PLC, kèm thông tin TSAP để định danh dịch vụ và vị trí CPU cần giao tiếp. Gói này được bọc bởi header TPKT, ví dụ:
\begin{quote}
\small\ttfamily
...
\texttt{Version: 3} là phiên bản TPKT theo RFC~1006 (giá trị cố định). \texttt{Length: 22} là tổng độ dài gói TPKT, gồm header TPKT (4 byte) và payload COTP CR (18 byte). PLC đọc 4 byte đầu để biết cần nhận đủ 22 byte rồi mới xử lý COTP; nếu gói bị cắt ngắn hoặc lỗi truyền, PLC có thể bỏ qua hoặc gửi lại yêu cầu kết nối.

Phần COTP CR điển hình (gói 8 trong bắt mẫu):
\begin{quote}
\small\ttfamily
ISO 8073/X.224 COTP Connection-Oriented Transport Protocol\\
\quad Length: 17\\
\quad PDU Type: CR Connect Request (0x0e)\\
\quad Destination reference: 0x0000\\
\quad Source reference: 0x000f\\
\quad Parameter code: src-tsap (0xc1), length: 2, Source TSAP: 0100\\
\quad Parameter code: dst-tsap (0xc2), length: 2, Destination TSAP: 0102\\
\quad Parameter code: tpdu-size (0xc0), length: 1, TPDU size: 1024
\end{quote}

Giải thích: \texttt{PDU Type: CR (0x0e)} là yêu cầu kết nối (ngoài ra còn \texttt{CC (0x0d)} và \texttt{DT (0x0f)}). \texttt{Destination reference} ban đầu bằng 0; \texttt{Source reference} do client tạo (\texttt{0x000f}). \texttt{Source TSAP: 0100}: byte \texttt{0x01} là PG (máy lập trình). \texttt{Destination TSAP: 0102}: Rack~0, Slot~2. \texttt{TPDU size: 1024} là kích thước TPDU tối đa client có thể gửi (TPDU = TPKT + PDU S7comm).

PLC phản hồi \textbf{COTP CC} (\textit{Connect Confirm}, gói 9):
\begin{quote}
\small\ttfamily
PDU Type: CC Connect Confirm (0x0d)\\
Destination reference: 0x000f\\
Source reference: 0x0003\\
Source TSAP: 0100, Destination TSAP: 0102, TPDU size: 1024
\end{quote}

\texttt{Destination reference: 0x000f} khớp \texttt{Source reference} của client; \texttt{Source reference: 0x0003} do PLC tạo --- client dùng giá trị này làm \texttt{Destination reference} trong các gói tiếp theo.

\subsubsection{Thiết lập S7comm (Setup Communication)}

Client gửi lệnh \textbf{Setup Communication} với \texttt{S7 Header Function Code = 0xF0}:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Figure/s7setup.png}
    \caption{Gói tin Setup Communication trong Wireshark}
    \label{fig:s7-setup}
\end{figure}

Yêu cầu Setup Communication (gói 10) có dạng:
\begin{quote}
\small\ttfamily
S7 Communication, Header: (Job)\\
\quad Protocol Id: 0x32, ROSCTR: Job (1)\\
\quad Reserved: 0x0000, PDU Reference: 512\\
\quad Parameter length: 8, Data length: 0\\
Parameter: Setup communication (0xf0)\\
\quad Max AmQ calling: 1, Max AmQ called: 1\\
\quad PDU length: 480
\end{quote}

\texttt{Max AmQ calling} quy định client gửi bao nhiêu request song song; \texttt{Max AmQ called} quy định PLC xử lý bao nhiêu request song song --- thường cả hai bằng 1 (client chỉ gửi yêu cầu mới sau khi nhận phản hồi trước). \texttt{PDU length: 480} là kích thước payload S7 tối đa client mong muốn (byte).

PLC phản hồi (gói 11):
\begin{quote}
\small\ttfamily
Header: (Ack\_Data), ROSCTR: Ack\_Data (3)\\
PDU Reference: 512, Error class/code: No error\\
Setup communication (0xf0), Max AmQ: 1/1, PDU length: 240
\end{quote}

\texttt{PDU Reference: 512} tham chiếu gói request; \texttt{Ack\_Data (3)} xác nhận thiết lập thành công; PLC chấp nhận tham số trừ \texttt{PDU length} giảm còn 240 byte. Wireshark có dissector S7comm tích hợp sẵn nên giải mã trực tiếp Parameter/Data~\cite{wireshark_s7comm}.

\subsubsection{Trao đổi dữ liệu}

Sau khi setup, hai bên trao đổi bằng các lệnh S7 như Read/Write Var, ReadSZL\ldots; tùy từng lệnh mà cấu trúc phần Parameter và Data sẽ khác nhau.

\subsection{Các vai trò trong giao tiếp S7}

Có ba vai trò chính trong giao tiếp S7:

\begin{itemize}
    \item \textbf{Client}: thường là HMI, SCADA hoặc Engineering Station, giám sát và điều khiển PLC. Client gửi lệnh đọc/ghi dữ liệu, yêu cầu thực thi chương trình hoặc thay đổi cấu hình PLC.

    \item \textbf{Server}: chính là PLC --- nhận yêu cầu, thực thi và trả kết quả (đọc/ghi, start/stop, trạng thái\ldots). PLC giao tiếp qua \textbf{CP} (\textit{Communication Processor}): nhận gói S7 từ mạng, giải mã, chuyển tới CPU; sau khi CPU xử lý, kết quả trả ngược qua CP. Các thiết bị bên trái (HMI, SCADA, máy lập trình) kết nối tới CP bên phải để gửi \textit{S7 Request}.

    PLC cũng có thể là \textbf{client} khi cần giao tiếp với PLC khác qua Get/Put để lấy dữ liệu hoặc gửi lệnh điều khiển.

    \item \textbf{Partner}: mô hình peer-to-peer --- sau khi thiết lập kết nối, cả hai bên đều có thể gửi dữ liệu mà không bắt buộc request--response từng gói như client--server. Trong tài liệu Siemens gọi là \textit{Client--Client}. PLC khởi tạo kết nối: \textit{Active Connection}; PLC nhận kết nối: \textit{Passive Connection}. Hai bên trao đổi qua BSend/BRecv: khi PLC~A gọi BSend thì PLC~B phải gọi BRecv cùng lúc để hoàn tất giao dịch.
\end{itemize}

\section{Hệ thống phát hiện xâm nhập (IDS)}
\label{sec:ids}

\subsection{Phương pháp xây dựng luật phát hiện}

IDS có thể phát hiện bất thường theo hai hướng: \textit{signature-based} (dựa chữ ký) và \textit{anomaly detection} (dựa hành vi/thống kê). Đồ án tập trung vào hướng chữ ký. Khi kiểm tra gói tin S7comm, có hai cách viết rule phổ biến:

\textbf{Mẫu byte (\textit{Byte Pattern / Raw Offset})}: hệ thống không cần hiểu ngữ nghĩa giao thức S7comm mà coi gói tin là chuỗi byte hex. Quy tắc chỉ định: tại vị trí thứ $X$, nếu thấy giá trị $Y$ thì báo động. Ví dụ: \texttt{(content:"|45 01|"; offset:22)} tương ứng byte liên quan chức năng mật khẩu của S7. Cách này nhanh, ít tốn tài nguyên vì làm việc trực tiếp trên tải TCP thô; nhược điểm là cứng nhắc --- nếu giao thức thay đổi hoặc vị trí byte lệch, luật có thể không còn hiệu lực.

\textbf{Từ khóa chuyên dụng (\textit{Protocol-Specific Keywords})}: yêu cầu parser hiểu sâu S7comm. Thay vì \texttt{offset:22}, có thể viết \texttt{s7comm\_function: password\_request}. Cơ chế: parser gom các mảnh TCP thành luồng hoàn chỉnh, chuyển chuỗi hex thành cấu trúc dữ liệu (mã chức năng, địa chỉ\ldots), rồi detector đối chiếu với rule.
\begin{itemize}
    \item Ưu điểm: chính xác, hiểu ngữ cảnh lệnh điều khiển.
    \item Nhược điểm: tốn CPU/RAM (\textit{overhead}) do thêm bước ``dịch'' từ mã máy sang cấu trúc dữ liệu~\cite{veit2021ics_dissectors}.
\end{itemize}
