...
Hình~\ref{fig:cksdownupprogramimagepng} minh họa pLC Siemens chạy trên PLC Sim Advance.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{../attacks/down_up_program/image.png}
    \caption{PLC Siemens chạy trên PLC Sim Advance}
...
Hình~\ref{fig:ksdownupprogramimage3png} minh họa chương trình PLC ở chế độ RUN với PLC Sim Advance và HMI WinCC.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{../attacks/down_up_program/image-3.png}
    \caption{Chương trình PLC ở chế độ RUN với PLC Sim Advance và HMI WinCC}
...
\paragraph{Các mẫu đại diện được chọn trong lab.}

Bảng~\ref{tab:malformed-phases} liệt kê mẫu payload, script và \texttt{sid} Suricata. Mỗi mẫu xuất phát từ một PDU \textbf{Setup Communication hợp lệ} (cùng hex với \texttt{s7comm\_dos.py}), chỉ \textbf{sửa một trường} (hoặc cắt) để sinh viên đối chiếu dễ trong Wireshark.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|l|l|}
        ...
        \hline
    \end{tabular}
    \caption{Mẫu malformed tiêu biểu và ánh xạ luật phát hiện}
    \label{tab:malformed-phases}
\end{table}

\paragraph{Triển khai script.}
