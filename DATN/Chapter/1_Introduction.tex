...
\begin{enumerate}
    ...
    \item Đồ án xây dựng môi trường mô phỏng phục vụ nghiên cứu các hành vi tấn công điển hình trên giao thức S7comm, bao gồm trinh sát, thao tác dữ liệu (ghi DB), DoS S7comm (\texttt{s7comm\_dos.py}), gói tin malformed (\texttt{s7comm\_malformed.py}), phát lại Start/Stop và quan sát MITM.
...
Chương 3 trình bày quá trình xây dựng môi trường mô phỏng và các kịch bản tấn công được sử dụng trong đồ án. Các kịch bản được lựa chọn nhằm thể hiện một số hành vi có rủi ro đối với hệ thống PLC, bao gồm thu thập thông tin, thay đổi dữ liệu quá trình, DoS (Mục~\ref{sec:dos_s7comm}), gói tin malformed (Mục~\ref{sec:malformed_plc}), phát lại lệnh điều khiển và can thiệp vào luồng truyền thông. Thông qua chương này, đồ án làm rõ cách tạo dữ liệu thực nghiệm phục vụ cho việc phân tích và phát hiện.

Chương 4 trình bày phương pháp phát hiện các hành vi tấn công S7comm bằng hệ thống IDS Suricata: phân tích cấu trúc gói tin, lựa chọn dấu hiệu và xây dựng luật trong \texttt{s7comm.rules}, \texttt{ics\_dos.rules} (Mục~\ref{sec:rules-dos}) và \texttt{s7comm\_malformed.rules} (Mục~\ref{sec:rules-malformed}).

Chương 5 trình bày thiết lập đánh giá lab và kết quả kiểm thử, trong đó có phát hiện DoS (\texttt{sid:1000040}--\texttt{1000043}, Mục~\ref{sec:eval-dos}) và malformed (\texttt{sid:1000050}--\texttt{1000058}, Mục~\ref{sec:eval-malformed}). Chương 6 tổng kết, nêu hạn chế và hướng phát triển.
