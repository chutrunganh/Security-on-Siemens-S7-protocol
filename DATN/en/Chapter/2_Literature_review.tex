...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\textwidth]{Figure/SCADA.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\textwidth]{Figure/DCS.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\textwidth]{Figure/SIS.png}
...
Figure~\ref{fig:plc-rack} shows rack and slot configuration in TIA Portal; rack~0, slot~1 is required to reach the correct communication module on a modular CPU.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.58\textwidth]{Figure/PLC_rack.png}
    \caption{TIA Portal rack and slot configuration}
    \label{fig:plc-rack}
\end{figure}

\subsection{PLC programming languages}

PLC control programs are written in five programming languages according to \textbf{IEC 61131-3}~\cite{iec61131_3}:
...
\begin{figure}
        \centering
        \includegraphics[width=0.8\textwidth]{Figure/Ladder.png}
        \caption{Self-holding circuit in Ladder}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|>{\raggedright\arraybackslash}p{2.1cm}|>{\raggedright\arraybackslash}p{4.3cm}|>{\raggedright\arraybackslash}p{7.2cm}|}
        ...
        PLC, RTU, Discrete Controller (in DCS). The PLCs themselves may not control robots directly, in most case, it send task to a lower level controller .e.g. robot controller. These controllers contain the actual robot control commands and execute the program corresponding to the task assigned by the PLC. \\
        \hline
        2 --- \textit{Supervisor Layer} &
        Contains local monitoring equipment for \textbf{one shop} in the plant. &
        Local HMI, local PC, shop Engineering Workstation. Each PCS often has one (or several for position convenience and redundancy) HMIs for operators \\
        \hline
        ...
        Database, SCADA Control Center, MES (Manufacturing Execution System, the system that coordinates production activities such as scheduling, order tracking, product quality monitoring). \\
        \hline
        3.5 --- \textit{DMZ Layer} &
        ...
        Firewall, data diodes, Jump Server, Replica Historian, Patch Server
        (since the OT network is not allowed to connect to the Internet, updates must be downloaded from Internet to Patch Server then OT machines get updates from this Patch Server instead of directly from the Internet). \\
        \hline
        4 --- \textit{Management Layer} &
        Internal IT network of the plant. &
        Personal/Company office PC/Laptop at the central control room, ERP (Enterprise Resource Planning, the system that manages all enterprise activities, including not only plant production (MES) but also HR, finance, and supply chain) \\
        \hline
        5 &
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Figure/S7_stack.png}
...
\subsection{Read addresses in Siemens S7}
...
The result is a protocol with clear message length (ISO advantage) that can also traverse network routers (TCP advantage). S7comm typically uses TCP port \textbf{102}. COTP is defined in RFC~905~\cite{rfc905}, TPKT in RFC~1006 and RFC~2126~\cite{rfc1006,rfc2126}.
...
\begin{table}[H]
    \centering
    \caption{S7comm packet types by ROSCTR~\cite{snap7_s7protocol,gmiru_s7comm_part1}}
    \label{tab:rosctr}
    \footnotesize
...
However, due to time and resource constraints, this thesis will only analyze and present the following command types~\cite{gmiru_s7comm_part2}:
...
Figure~\ref{fig:s7-connect-flow} shows the connection setup sequence from the Wireshark sample capture \texttt{s7comm\_reading\_plc\_status.pcap}~\cite{wireshark_s7comm} (filter \texttt{tcp.port==102}).

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Figure/setup_S7.png}
    \caption{S7comm connection setup flow}
...
Figure~\ref{fig:s7-tcp-handshake} shows the TCP three-way handshake before S7comm.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{image.png}
    \caption{TCP three-way handshake to port 102}
...
Figure~\ref{fig:s7-cotp-cr} shows the COTP Connect Request (packet~8).

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{image-2.png}
    \caption{COTP Connect Request (CR)}
...
\begin{itemize}
    ...
    Figure~\ref{fig:s7-client-server} shows clients on the left sending S7 requests to the CP on the right.

\begin{figure}[H]
        \centering
        \includegraphics[width=0.58\textwidth]{Siemens_S7_Client_Server.png}
        \caption{S7 client--CP connection model}
...
\begin{figure}[H]
        ...
        \caption{Internal PLC server architecture}
...
Signature-based detection compares observed data against predefined rules. A rule may describe a byte sequence in a payload, a value in a protocol header, a URL path, or a sequence of commands associated with a known attack. For ICS protocols, Veit compares rules that match raw TCP payload bytes with rules that use protocol-specific dissectors and reports trade-offs in accuracy and performance~\cite{veit2021ics_dissectors}. Matching is deterministic and easy to audit: given a rule and a packet capture, an analyst can usually reproduce the alert. The limitation is coverage---only attacks that have been described in advance will match.
