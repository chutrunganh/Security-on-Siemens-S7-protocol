...
Across ten attack classes, $R_{\mathrm{det}} = 10/10 = 100\%$ (Table~\ref{tab:eval-detection-summary}). This figure measures whether the rule set \emph{reacts} to each attack class under the one-SID criterion of Section~\ref{sec:eval-setup}; it is not a measure of accuracy against an adversary who actively evades the signatures, and Section~\ref{sec:eval-discussion} qualifies how it should be read. The subsections below interpret each group, with the per-scenario counts in Table~\ref{tab:eval-detection-summary}.
...
The DoS scenario produced 5\,850 alerts. Rate-based rules \texttt{1000040} (TCP SYN churn) and \texttt{1000042} (Read SZL flood) fired; the Setup-only flood (\texttt{1000041}) and generic Job flood (\texttt{1000043}) did not. The reason is specific to this run: the flooding phases that reached their per-source thresholds were the SYN churn and the Read SZL flood, whereas the Setup and mixed-Job rates stayed below the 25- and 80-packet counts in Table~\ref{tab:eval-dos-thresholds}. Opcode rules such as \texttt{1000001} and \texttt{1000002} also fired on the individually valid flooded PDUs; they add context but do not substitute for the rate-based alerts. Comparing Table~\ref{tab:eval-dos-thresholds} with the baseline counts shows why benign polling stayed below every threshold while the flood phase crossed two of them.
...
\paragraph{Start/Stop command injection.}
The injected Stop command triggered \texttt{1000032} and \texttt{1000033}; the injected Start command triggered \texttt{1000030} and \texttt{1000031}. In both cases the generic opcode rule and the \texttt{P\_PROGRAM} string variant fired on the fixed payloads from Section~\ref{sec:start_stop_plc}. Because the payload is identical to a legitimate command, these rules also match authorised maintenance unless source IP or a change-window policy is applied.
