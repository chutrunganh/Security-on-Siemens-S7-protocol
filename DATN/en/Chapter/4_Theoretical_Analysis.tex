...
This chapter defines the Suricata rule set used to detect S7comm attack behaviour documented in Chapter~\ref{sec:attack-scenarios}. Detection is signature-based: each rule tests for a fixed byte pattern in the TCP/102 payload spanning TPKT, COTP, the S7 header, and---where applicable---function codes or rate limits.

Rules are stored in \texttt{detect/rules/s7comm.rules}, \texttt{detect/rules/ics\_dos.rules}, and \texttt{detect/rules/s7comm\_malformed.rules}. SID ranges group related behaviour: \texttt{100000x} (session setup and reconnaissance), \texttt{100001x} (read/write variables), \texttt{100002x} (block upload/download), \texttt{100003x} (Start/Stop control), \texttt{100004x} (DoS and rate floods), \texttt{100005x} (malformed or syntactically invalid PDUs).

A rule is more than a Wireshark export. Each signature must specify (i)~which encapsulation bytes are checked, (ii)~traffic direction relative to the PLC on port~102, and (iii)~whether a single match or a \texttt{threshold} counter is appropriate. The chapter follows that order: shared offsets, PCAP-to-rule workflow, then one subsection per Chapter~3 scenario.

\section{From captured traffic to Suricata signatures}
\label{sec:pcap-to-rules}

Every signature in this chapter was derived from PCAP files captured while running the Chapter~3 scenarios. The same five-step procedure was applied to reconnaissance, Start/Stop replay, upload/download, Write Var and MITM, DoS, and malformed traffic.

\begin{enumerate}
    \item \textbf{Capture.} Execute the scenario script (\texttt{nmap --script s7-info}, \texttt{scanBlock.py}, TIA Portal replay, \texttt{s7comm\_dos.py}, \texttt{s7comm\_malformed.py}, Snap7 Write Var, or Bettercap MITM) and record traffic on the IDS mirror interface or the attacker host so each TCP/102 segment retains its full payload.

    \item \textbf{Locate anchors in Wireshark.} Valid S7comm Data TPDUs begin with TPKT version \texttt{03 00} at offset~0, COTP Data TPDU \texttt{02 F0 80} at offset~4, and S7 protocol identifier \texttt{32} at offset~7. Missing anchors indicate non-S7 traffic or malformed samples (Section~\ref{sec:rules-malformed}).
    ...
    \item \textbf{Choose single-match vs.\ rate counting.} One Setup or Write Var per session is normal engineering traffic and is detected with a plain \texttt{content} rule (\texttt{sid:1000001}, \texttt{sid:1000010}). DoS scripts repeat valid PDUs at high frequency; those signatures reuse the same bytes but add \texttt{threshold:type both, track by\_src, count N, seconds 10}. Malformed injections alter one field; \texttt{content}, \texttt{byte\_test}, or \texttt{dsize} on that field suffices without a threshold.

    \item \textbf{Validate offline.} Run \texttt{suricata -r scenario.pcap} with the relevant rule files and compare \texttt{fast.log} SIDs and timestamps to Wireshark packet numbers. Missed alerts usually trace to wrong offset (17 vs.\ 19), inverted flow direction, or capture on an interface that never saw the attack path.
\end{enumerate}

The same function byte can therefore appear in multiple rules with different \texttt{flow} and \texttt{ROSCTR} tests---for example \texttt{0x1b} in \texttt{sid:1000021} (Job from PLC) and \texttt{sid:1000026} (Ack\_Data to PLC) reflects opposite legs of the download handshake in Section~\ref{sec:down_up_program}.

\section{Common rules for identifying the S7comm protocol}
\label{sec:rule-foundation}

On TCP/102, S7comm rides inside TPKT and COTP before the S7 header. Table~\ref{tab:s7-payload-offsets} lists the payload offsets referenced throughout the rule files.
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|p{7.2cm}|}
        \hline
        \textbf{Offset} & \textbf{Value} & \textbf{Field} \\
        \hline
...
Bytes~2--3 hold the TPKT length field encoded in big-endian. Opcode rules ignore this field, but malformed detection (\texttt{sid:1000051}, \texttt{sid:1000058}) compares the declared length against the actual TCP segment size via \texttt{byte\_test} and \texttt{dsize}. Using a single offset table avoids re-counting bytes when moving from Wireshark to Suricata syntax.
